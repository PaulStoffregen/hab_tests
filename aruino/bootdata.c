...
extern unsigned long __image_size;
extern unsigned long _flashimagelen;
extern unsigned long _scsf;
...
__attribute__ ((section(".pad"), used))
const uint32_t pad = 0xDEADC0DE;
...
const uint32_t BootData[4] = {
	0x60000000,
	(uint32_t)&__image_size,
	0,
	0,
};
...
const uint32_t ImageVectorTable[8] = {
	0x432000D1,		// header
	(uint32_t)&ResetHandler, // docs are wrong, needs to be vec table, not start addr
	0,			// reserved
	(uint32_t)dcd,			// dcd
	(uint32_t)BootData,	// abs address of boot data
	(uint32_t)ImageVectorTable, // self
	(uint32_t)&_scsf,		// command sequence file
	0			// reserved
};
